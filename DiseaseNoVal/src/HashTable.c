...
HashTable initHashTable(int bucket_size, int entries){
    ...
    if(array_size==0){
        printf("Bucket_Size should be at least 16 bytes to store 2 void pointers and 1 next bucket pointer.\n");
        free(HT);
        return NULL;
    }
        ...
        for(int i=0; i<HtB->totalValues; i+=2){
            if(HtB->arr[i]!=NULL){
                if(strcmp(HtB->arr[i], countr_or_disease)==0){  // Found specific country or disease
                    ...
                    reheapify( &(MaxHeapTree->root) );  // reheapify CAN BE AFTER EVERY NODE OR IN THE END
                    
                    // printf("\tAVL tree of bucket\n");
                    // printAVLTree(tmpAVL); printf("\n");
                    
                    // printf("Heap created:\n");
                    // printMaxHeapNode(MaxHeapTree->root, 0); printf("\n");
